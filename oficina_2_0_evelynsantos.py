# -*- coding: utf-8 -*-
"""oficina_2_0_evelynsantos.ipynb
ETAPA 1
...
def compare(user, machine):
    if user==machine:
        return "Empate!"
    if ((user == 0 and machine == 2) or
        (user == 2 and machine == 1) or
        (user == 1 and machine == 0)):
      return 'Usuário ganhou!'
    else:
      return 'Máquina Ganhou!'

def machine(options):
    playRandom = random.choice(options)
    return playRandom, options.index(playRandom) 

def validIpt(opt, max, min):
   return min <= opt <= max   
...
while True:
    print(menu)
    playMachine, choiceMachine = machine(options)
    ...
    if validIpt(playUser, 4, 1):
        if playUser == 4:
            break
        
        print(f'Sua jogada -> {options[playUser-1]}')
        print(f'Sua jogada -> {playUser}')

        print(f'Jogada da máquina -> {playMachine}') 
        print(f'Jogada da máquina -> {choiceMachine+1}')         
        print(compare(playUser-1, (choiceMachine)))
